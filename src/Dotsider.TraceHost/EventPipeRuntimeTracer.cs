...
/// Runs only inside the framework-dependent trace host process.
/// </summary>
...
/// <param name="eventCaptured">The callback that receives each captured runtime event.</param>
/// <param name="countersUpdated">The callback that receives each counter snapshot.</param>
/// <param name="outputCaptured">The callback that receives each captured output line.</param>
internal sealed class EventPipeRuntimeTracer(
    string assemblyPath,
    IReadOnlyList<string> arguments,
    Action invalidate,
    Action<TraceEventEntry> eventCaptured,
    Action<CounterSnapshot> countersUpdated,
    Action<OutputLine> outputCaptured) : IDisposable
{
    private const int MaxEvents = 10_000;
    private const int MaxOutputLines = 5_000;
    private const int MaxOutputLineLength = 64 * 1024;
    private const string OutputTruncationMarker = " … [truncated]";
    ...
    private readonly Action<TraceEventEntry> _eventCaptured = eventCaptured
        ?? throw new ArgumentNullException(nameof(eventCaptured));
    private readonly Action<CounterSnapshot> _countersUpdated = countersUpdated
        ?? throw new ArgumentNullException(nameof(countersUpdated));
    private readonly Action<OutputLine> _outputCaptured = outputCaptured
        ?? throw new ArgumentNullException(nameof(outputCaptured));
    ...
    private string? _diagnosticPortDirectory;
    ...
    public void Start()
    {
        ...
        var psi = CreateStartInfo(_diagnosticPort);
        ...
        _processingTask = Task.Factory.StartNew(async () =>
        {
            try
            {
                ...
                if (session is null)
                {
                    lock (_stateLock)
                    {
                        var seconds = ConnectTimeout.TotalSeconds.ToString(
                            "0",
                            CultureInfo.InvariantCulture);
                        _errorMessage =
                            $"Timed out starting runtime diagnostics after {seconds}s. " +
                ...
                lock (_stateLock)
                {
                    if (_processState == TraceProcessState.Starting)
                        _processState = TraceProcessState.Running;
                }
                MarkDirty();
            ...
            catch (OperationCanceledException)
            {
                lock (_stateLock)
                {
                    var seconds = ConnectTimeout.TotalSeconds.ToString(
                        ...
                        CultureInfo.InvariantCulture);
                    _errorMessage =
                        $"Timed out connecting to runtime diagnostics after {seconds}s. " +
                        "Is this a valid .NET assembly?";
                    _processState = TraceProcessState.Error;
                }
    ...
    internal ProcessStartInfo CreateStartInfo(string diagnosticPort)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(diagnosticPort);

        var requiresDotnetHost = _assemblyPath.EndsWith(
            ".dll",
            StringComparison.OrdinalIgnoreCase);
        var startInfo = new ProcessStartInfo
        {
            FileName = requiresDotnetHost ? ResolveCurrentDotNetHost() : _assemblyPath,
            RedirectStandardError = true,
            RedirectStandardOutput = true,
            UseShellExecute = false
        };

        if (requiresDotnetHost)
        {
        ...
        ConfigureDiagnosticsEnvironment(startInfo.Environment, diagnosticPort);

        return startInfo;
    }

    internal static void ConfigureDiagnosticsEnvironment(
        ...
        string diagnosticPort)
    {
        ...
        environment["DOTNET_DiagnosticPorts"] = diagnosticPort;
        environment.Remove("DOTNET_DefaultDiagnosticPortSuspend");
        environment.Remove("DOTNET_EnableDiagnostics");
        environment.Remove("DOTNET_EnableDiagnostics_IPC");
        environment.Remove("COMPlus_EnableDiagnostics");
        environment.Remove("COMPlus_EnableDiagnostics_IPC");
    }
    ...
    internal static string ResolveCurrentDotNetHost()
    {
        ...
        throw new InvalidOperationException(
            "The trace host could not resolve the .NET host that launched it.");
    }

    internal static string CreateDiagnosticPort(out string? directoryPath)
    {
        ...
        var directory = Directory.CreateTempSubdirectory("ds-");
    ...
    private CounterSnapshot? ParseCounterEvent(TraceEvent data)
    {
        ...
        int metadataToken = 0)
    {
        ...
        _eventCaptured(entry);
        MarkDirty();
    }

    internal void ReadOutput(StreamReader reader, bool isStdErr, Stopwatch timer)
    {
        try
        {
    ...
    {
        var text = truncated
            ? string.Concat(line.ToString(), OutputTruncationMarker)
            : line.ToString();
        var output = new OutputLine(timer.Elapsed, isStdErr, text);
        _outputQueue.Enqueue(output);
        while (_outputQueue.Count > MaxOutputLines)
            _outputQueue.TryDequeue(out _);
        _outputCaptured(output);
        MarkDirty();
    }
