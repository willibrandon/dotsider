...
Native DWARF materialization from ELF images and dSYM bundles is limited to 256 MiB total per symbol read. Oversized or malformed compressed ELF sections do not abort analysis: `--symbols` reports corrupt symbol data and uses `.eh_frame` boundaries when the image contains them.
