...
public class TreemapLayoutTests
{
    ...
    public void Layout_ProducesRectsWithinBounds()
    {
        ...
        {
            Assert.IsGreaterThanOrEqualTo(-0.01, rect.X);
            Assert.IsGreaterThanOrEqualTo(-0.01, rect.Y);
            Assert.IsLessThanOrEqualTo(100.01, rect.X + rect.Width);
            Assert.IsLessThanOrEqualTo(100.01, rect.Y + rect.Height);
        }
    }
    ...
    public void Layout_NoNegativeDimensions()
    {
        ...
        TestAssert.All(rects, r =>
        {
            Assert.IsGreaterThanOrEqualTo(0, r.Width);
            Assert.IsGreaterThanOrEqualTo(0, r.Height);
        });
    }
    ...
    public void Layout_SingleNode_FillsEntireSpace()
    {
        ...
        Assert.ContainsSingle(rects);
        Assert.AreEqual(0, rects[0].X, 0.01);
        Assert.AreEqual(0, rects[0].Y, 0.01);
        Assert.AreEqual(50, rects[0].Width, 0.01);
        Assert.AreEqual(30, rects[0].Height, 0.01);
    }
    ...
    public void Layout_ZeroWidthOrHeight_ReturnsEmpty()
    {
        ...
        Assert.IsEmpty(TreemapLayout.Layout(nodes, 0, 0, 0, 100));
        Assert.IsEmpty(TreemapLayout.Layout(nodes, 0, 0, 100, 0));
    }
    ...
    public void Layout_ManySmallNodes_AllFitWithinBounds()
    {
        ...
        foreach (var rect in rects)
        {
            Assert.IsGreaterThanOrEqualTo(9.99, rect.X);
            Assert.IsGreaterThanOrEqualTo(9.99, rect.Y);
            Assert.IsLessThanOrEqualTo(210.01, rect.X + rect.Width);
            Assert.IsLessThanOrEqualTo(160.01, rect.Y + rect.Height);
        }
    }
    ...
    public void AssertNoOverlaps_DetectsKnownOverlap()
    {
        ...
        var ex = Assert.Throws<Exception>(() => AssertNoOverlaps(overlapping));
    ...
    public void Layout_RectsMatchInputNodes()
    {
        var nodes = CreateTestNodes(5);
        var rects = TreemapLayout.Layout(nodes, 0, 0, 100, 100);
        Assert.HasCount(nodes.Count, rects);
    }
    ...
    private static void AssertFullCoverage(IReadOnlyList<TreemapRect> rects, double width, double height)
    {
        ...
        Assert.AreEqual(width * height, totalRectArea, 0.01);

        var maxRight = rects.Max(r => r.X + r.Width);
        Assert.AreEqual(width, maxRight, 0.01);

        var maxBottom = rects.Max(r => r.Y + r.Height);
        Assert.AreEqual(height, maxBottom, 0.01);
    }
