...
public class XarchAvxTests
{
    ...
    [DataRow("vaddps", "ymm0, ymm1, ymm2", new byte[] { 0xC5, 0xF4, 0x58, 0xC2 })]
    [DataRow("vaddsd", "xmm0, xmm1, xmm2", new byte[] { 0xC5, 0xF3, 0x58, 0xC2 })]
    [DataRow("vpxor", "xmm0, xmm1, xmm2", new byte[] { 0xC5, 0xF1, 0xEF, 0xC2 })]
    [DataRow("vmovaps", "ymm0, ymm1", new byte[] { 0xC5, 0xFC, 0x28, 0xC1 })]
    [DataRow("vmovdqa", "ymm0, ymmword ptr [rcx]", new byte[] { 0xC5, 0xFD, 0x6F, 0x01 })]
    [DataRow("vbroadcastss", "ymm0, xmm1", new byte[] { 0xC4, 0xE2, 0x7D, 0x18, 0xC1 })]
    [DataRow("vinsertf128", "ymm0, ymm0, xmm2, 0x1", new byte[] { 0xC4, 0xE3, 0x7D, 0x18, 0xC2, 0x01 })]
    [DataRow("vzeroupper", "", new byte[] { 0xC5, 0xF8, 0x77 })]
    [DataRow("vzeroall", "", new byte[] { 0xC5, 0xFC, 0x77 })]
    [DataRow("vpmulld", "ymm0, ymm1, ymm2", new byte[] { 0xC4, 0xE2, 0x75, 0x40, 0xC2 })]
    [DataRow("vpsllvd", "xmm0, xmm1, xmm2", new byte[] { 0xC4, 0xE2, 0x71, 0x47, 0xC2 })]
    [DataRow("vcvtph2ps", "ymm0, xmm1", new byte[] { 0xC4, 0xE2, 0x7D, 0x13, 0xC1 })]
    ...
    public void Decode_Avx256_UsesYmmAndVectorCategory()
    {
        ...
        Assert.AreEqual(NativeInstructionCategory.Vector, insn.Category);
    }
}
