...
/// <summary>
/// Verifies deterministic lifetime behavior for the test Unix domain socket servers.
/// </summary>
[TestClass]
public sealed class TestSocketLifecycleTests
{
    ...
    public async Task RawJsonSocket_ConcurrentShutdownWhileAccepting_IsIdempotent()
    {
    ...
    /// <summary>
    /// Verifies a handler fault that precedes shutdown is surfaced by disposal.
    /// </summary>
    [TestMethod]
    [Timeout(30_000, CooperativeCancellation = true)]
    public async Task RawJsonSocket_HandlerFaultBeforeShutdown_IsPropagated()
    {
        var socketPath = GetUniqueSocketPath();
    ...
    public async Task DotsiderSocket_ShutdownShapedHandlerFault_IsPropagated(string faultKind)
    {
        ...
        var handlerEntered = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        using var releaseHandler = new ManualResetEventSlim();
        var server = new TestDotsiderSocket(socketPath);
        server.On(
            "fail",
            _ =>
            {
    ...
    /// <summary>
    /// Verifies an explicitly disposed server cannot start accepting connections.
    /// </summary>
    [TestMethod]
    [Timeout(30_000, CooperativeCancellation = true)]
    public async Task RawJsonSocket_StartAfterShutdown_ThrowsObjectDisposedException()
    {
        var server = new TestRawJsonSocket(GetUniqueSocketPath());
        await server.DisposeAsync();

        _ = Assert.ThrowsExactly<ObjectDisposedException>(server.Start);
    }
