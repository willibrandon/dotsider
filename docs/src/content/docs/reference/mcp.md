...
Native DWARF materialization from ELF images and dSYM bundles is limited to 256 MiB total per symbol read. Oversized or malformed compressed ELF sections leave the remaining analysis available; `get_native_symbols` reports corrupt symbol data and returns `.eh_frame` boundaries when present.
