...
public class NativeResolutionTests
{
    ...
    public void RipRelative_ComputesAbsoluteDataTarget()
    {
        ...
        Assert.AreEqual(0x1007UL, insn.TargetAddress);
        Assert.AreEqual(NativeTargetKind.Data, insn.TargetKind);
    }
    ...
    public void IntraWindowBranch_BecomesLocalLabel()
    {
        ...
        Assert.AreEqual(0x1002UL, insns[0].TargetAddress);
        Assert.AreEqual(NativeTargetKind.LocalLabel, insns[0].TargetKind);
        Assert.AreEqual("loc_1002", insns[0].TargetName);
    }
    ...
    public void ResolvedCall_RendersNameAndOffset()
    {
        ...
        Assert.AreEqual("Foo", insns[0].TargetName);
        Assert.AreEqual("Foo+0x5", insns[1].TargetName);
        Assert.AreEqual(NativeTargetKind.Function, insns[0].TargetKind);
    }
}
