...
public class IlcNameDemanglerTests
{
    ...
    public void Demangle_EntryPoint_JoinsExactly()
    {
        ...
        Assert.AreEqual("Program.<Main>$", result.ManagedName);
        Assert.AreEqual(NativeSymbolKind.Function, result.Kind);
    ...
    public void Demangle_GenericInstanceMethod_JoinsAfterScopeStrip()
    {
        ...
        Assert.AreEqual("System.ReadOnlySpan`1.ToString", result.ManagedName);
        Assert.IsTrue(result.IsExactMatch);
    }
    ...
    public void Demangle_WindowsVtable_ClassifiesAsMethodTable()
    {
        ...
        Assert.AreEqual(NativeSymbolKind.MethodTable, result.Kind);
        Assert.AreEqual("System.Collections.Generic.StringEqualityComparer (MethodTable)", result.ManagedName);
        Assert.IsTrue(result.IsExactMatch);
    }
    ...
    public void Demangle_UnixVtable_StripsLengthPrefix()
    {
        ...
        Assert.AreEqual(NativeSymbolKind.MethodTable, result.Kind);
        Assert.AreEqual("System.Object (MethodTable)", result.ManagedName);
    }
    ...
    [TestMethod]
    [Timeout(30_000, CooperativeCancellation = true)]
    [DataRow("?__GCSTATICS@S_P_CoreLib_System_Text_EncoderReplacementFallback@@", NativeSymbolKind.Statics)]
    [DataRow("__NONGCSTATICS_SomeType", NativeSymbolKind.Statics)]
    [DataRow("__TypeThreadStaticIndex_SomeType", NativeSymbolKind.Statics)]
    [DataRow("__GenericDict_S_P_CoreLib_System_Array__Resize", NativeSymbolKind.GenericDictionary)]
    [DataRow("__writableDataString", NativeSymbolKind.Data)]
    [DataRow("__readonlydata_SomeMethod", NativeSymbolKind.Data)]
    [DataRow("_MyModule__Str_48656C6C6F", NativeSymbolKind.FrozenObject)]
    [DataRow("__unbox_SomeType", NativeSymbolKind.Stub)]
    ...
    public void Demangle_NestedDisplayClass_UnderscoreJoined()
    {
        ...
        Assert.AreEqual("System.Foo+<>c.<Bar>b__0_0", result.ManagedName);
        Assert.IsTrue(result.IsExactMatch);
    }
    ...
    public void Demangle_KnownTypeUnknownMethod_ClaimsNoManagedName()
    {
        ...
        Assert.IsNull(result.ManagedName);
        Assert.IsFalse(result.IsExactMatch);
    }
    ...
    public void Demangle_DuplicateMethodName_SharedNameNeverExact()
    {
        ...
        Assert.AreEqual("System.Foo.Bar", result.ManagedName);
        Assert.IsFalse(result.IsExactMatch);
    }
    ...
    {
        Assert.AreEqual("Foo_Bar", IlcNameDemangler.Sanitize("Foo.Bar"));
        Assert.AreEqual("_1Type", IlcNameDemangler.Sanitize("1Type"));
        Assert.AreEqual("List_1", IlcNameDemangler.Sanitize("List`1"));
        Assert.AreEqual("_Main__", IlcNameDemangler.Sanitize("<Main>$"));
        Assert.AreEqual("a_b", IlcNameDemangler.Sanitize("aéb")); // é → one underscore
        Assert.AreEqual("x_y", IlcNameDemangler.Sanitize("x\U0001F600y")); // emoji (surrogate pair) → one underscore
    }
}
