...
| `NativeDisassembler` | Disassembles a native code window into `NativeInstruction`s and a rendered listing for x86-64 (legacy → SSE/AVX/AVX-512 via a table-driven, VEX/EVEX-aware decoder) and AArch64 (base → AdvSIMD/crypto → SVE/SVE2), with exact instruction length by construction. `DisassembleSymbol` slices one recovered `NativeSymbol`, resolves call/branch/data targets to names (`Foo+0x12`, `loc_…`, `MODULE!Function` via the `NativeImportResolver`), and stamps per-instruction source from the `NativeSourceMap` |
