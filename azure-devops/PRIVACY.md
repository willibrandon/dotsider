...
Unless `dotsiderPath` is supplied, the task contacts GitHub to resolve or download an official Dotsider release and its checksum. To manage baselines, it uses the pipeline's short-lived SystemVssConnection token to read successful builds and artifacts from the current Azure DevOps project. For an Azure Repos pull request, it inspects the checked-out `Build.SourceVersion` commit locally first. Only when that commit object is unavailable does it use the Azure Repos commit API, which requires the pipeline Build Service identity to have repository Read permission. The token is removed from the environment before Dotsider runs and is never written to reports or logs.
