...
public class NativeDecoderRegistryTests
{
    ...
    {
        Assert.False(NativeDecoderRegistry.TryDecode(
            NativeArchitecture.Unknown, [0x40], 0, 0x1000, out _));

        var insn = Assert.Single(NativeDisassembler.Disassemble([0x40], 0x1000, NativeArchitecture.Unknown));
        Assert.Equal(".byte", insn.Mnemonic);
